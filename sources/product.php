<?php  if(!defined('_source')) die("Error");
	...
	else if($id!='')
	{
		...
		//Chi tiết sản phẩm
		$d->reset();
		$sql_detail = "select * FROM #_product where hienthi=1 and id='$id' limit 0,1";
		$d->query($sql_detail);
	...
	$d->reset();
	$sql = "select id,ten$lang as ten,tenkhongdau,thumb,photo,masp,gia,giacu,mota$lang as mota,file,hangsanxuat$lang as hsx,luotxem FROM #_product where $where limit $bg,$pageSize";		
	$d->query($sql);
